...
a=this.content_param+">:not([data-jets"+(this.options.searchSelector||"*")+'="'+b+'"]){display:none'+(this.options.addImportant?"!important":"")+"}";this.styleTag.innerHTML=b?a:""},_addStyleTag:function(){this.styleTag=document.createElement("style");this.search_tag.parentNode.insertBefore(this.styleTag,this.search_tag)},_getText:function(b){return b&&(b.innerText||b.textContent)||""},_getContentTags:function(b){return Array.prototype.slice.call(this.content_tag.querySelectorAll(b||":scope > *"))},
_setJets:function(b,a){for(var c=this,h=c._getContentTags(a?"":b),d,g=0,e;e=h[g];g++)if(!e.hasAttribute("data-jets")||a)d=this.options.manualContentHandling?this.options.manualContentHandling(e):c.options.columns&&c.options.columns.length?c.options.columns.map(function(b){return c._getText(e.children[b])}).join(" "):c._getText(e),e.setAttribute("data-jets",d.trim().replace(/\s+/g," ").toLowerCase())},update:function(b){this._setJets(":scope > :not([data-jets])",b)},_destroy:function(){this.styleTag.parentNode&&
this.styleTag.parentNode.removeChild(this.styleTag);for(var b=this._getContentTags(),a=0,c;c=b[a];a++)c.removeAttribute("data-jets")}};(function(b,a){try{b.querySelector(":scope body")}catch(c){["querySelector","querySelectorAll"].forEach(function(c){var d=a[c];a[c]=function(a){if(/(^|,)\s*:scope/.test(a)){var e=this.id;this.id="ID_"+Date.now();a=a.replace(/((^|,)\s*):scope/g,"$1#"+this.id);a=b[c](a);this.id=e;return a}return d.call(this,a)}})}})(window.document,Element.prototype);return f});